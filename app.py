...
# --- 📥 Load data (default or user upload) ---
uploaded_file = st.file_uploader("Upload your own transaction CSV (optional)", type="csv")

@st.cache_data
def load_data():
    return pd.read_csv("category_merchant_data.csv")
...
st.success("✅ Data loaded from " + ("uploaded file." if uploaded_file else "default dataset."))
...
cat_summary = (
    ...
    .agg(txn_count=('id', 'count'), total_spend=('amount_abs', 'sum'), avg_txn=('amount_abs', 'mean'))
